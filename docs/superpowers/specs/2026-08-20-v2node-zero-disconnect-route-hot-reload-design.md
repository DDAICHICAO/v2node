...
- 路由切换期间的新连接失败数为零；每条新连接的 Router 与 Outbound 映射只能来自完整可用的旧代次或新代次，单次 DNS 查询也只能使用一个完整可用的后端。
...
1. 切换前已建立的连接继续使用其创建时取得的 Router 和 Outbound 代次，直到连接自然结束；DNS 查询遵循后文单次查询边界。
2. 切换期间到达的新连接可以在极短的发布临界区等待，但不能因 Router、DNS 或 Outbound 暂时缺失而失败；Router 与 Outbound 按连接线性化，DNS 后端按单次查询线性化。

运行配置发布具有代次线性化点。Dispatcher 在该点之前取得旧 Router/Outbound 代次，在该点之后取得新代次。已取得代次的连接不会在中途自动迁移，也不会因后续代次发布而失效。
...
严格编译同时用于最后可用快照恢复。为了避免升级时把历史上被旧逻辑静默忽略的错误规则突然变成启动故障，首次运行新版本且尚无整机最后可用快照时，初始 Core 仍按现有兼容语义启动，并把实际运行对象包装为 generation 0。启动后立即对当前 observed 配置执行严格编译预检：成功后才能提升为已验证快照；失败时继续运行 generation 0，将热更新状态标记为 degraded 并持续告警、重试，不能发布残缺候选。第一次严格验证成功后，后续恢复统一使用最后可用快照。收紧首次启动的历史兼容语义属于单独治理范围。
...
11. 并发 Acquire 与 Publish 时，连接的 Router 与 Outbound 只能取得完整旧代次或完整新代次。
...
2. 部署前用严格编译器预检该机器当前全部路由；发现历史错误时先修正规则，不把首次兼容启动当成长期正常状态。
3. 先在一台配置多个 NodeID 的机器启用默认热更新路径。
4. 部署前建立持续 TCP、UDP 和新连接探针，记录基线。
5. 连续执行覆盖 DNS、block、自定义 Outbound 和默认出口的路由编辑。
6. 确认连接失败数为零、Core 和监听器没有重启、一个批次只发布一个代次。
7. 观察 retired generation、CPU、内存、goroutine、心跳和错误指标。
8. 单机稳定后逐步扩大范围；只有出现无法通过保持旧代次规避的实现级故障时，才人工关闭热更新开关。
