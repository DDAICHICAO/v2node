...
- 正常突发只形成短暂批量等待；数据面等待本地持久化结果的硬上限为 1 秒。超过 1 秒时，如果事件已经进入有界单写队列，则返回“结果待定”并继续由原写入循环提交，只累计 `persist_timeouts` 延迟指标；只有队列饱和导致事件未被接收、事务真实失败或关闭后拒绝接收时才进入 hard-failure/gap 统计。不得为超时事件创建新的后台 goroutine。
...
3. bbolt 在 1 秒内提交成功则返回成功；若事件已被队列接收但结果仍待定，调用方按已排队处理，最终成功后再唤醒上传器。
...
普通访问记录不再以 channel 满作为静默丢弃条件。若 1 秒内无法得到提交结果，但事件已经进入有界队列，则调用按已排队返回、累计延迟指标并输出独立的限频 warning，不能把未知结果误记为日志缺口。若事件在超时前仍未进入队列，或 bbolt 事务最终真实失败，则累计 persistence gap 并输出限频错误。这个 hard-failure 分支继续放行业务流量，以符合已确认的节点可用性边界，但日志状态必须明确显示该时间段存在缺口。
